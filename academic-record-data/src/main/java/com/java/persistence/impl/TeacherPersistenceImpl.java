...
public class TeacherPersistenceImpl implements TeacherPersistence {
    ...
    @Override
    public Collection<Teacher> getAll() throws ExceptionPersistence {
        try{
            if (teacherRepository.count() != 0){
                return teacherRepository.findAll();
            }
        } catch (Exception e){
            throw new ExceptionPersistence("Failed to get All Teachers");
        }
        return teacherRepository.findAll();
    }
    ...
    @Override
    public void update(Teacher teacher, Integer id) throws ExceptionPersistence {
        try {
            teacherRepository.save(teacher);
        } catch (Exception e){
